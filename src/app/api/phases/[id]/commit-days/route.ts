...
export async function POST(
  ...
  // NOTE: phase_exercises.day_id is NOT written here. In the pinning model the
  // column is the source of truth and is persisted the moment an assignment
  // changes (PATCH /exercises on assign/move, POST on add/copy). commit-days
  // only syncs the split config + the relational day/day_exercise tables, so it
  // must never touch phase_exercises — doing so was redundant and broke saving.
